...
class RansomNote extends HTMLElement {
  constructor() {
    ...
    this._mytext = this.innerHTML
    // Split the string into an array of words text.split(' ')
    this._splitToArray = this._mytext.split('')
    this._newArray = []
    // Loop over each word in the array
    for (let i = 0; i < this._splitToArray.length; i += 1) {
      const element = document.createElement('span')
      element.style.display = "inline-block"
      element.innerHTML = this._splitToArray[i]
      element.style.fontSize = (Math.random() * 30 + 20) + 'px'
      element.style.fontWeight = Math.random() * 6 * 100 
      const color = ['red', 'blue', 'yellow', 'green', 'orange']
      element.style.color = color[Math.floor(Math.random() * color.length)]

      const myborderStyles = ['none', 'solid', 'double', 'dotted', 'dashed', 'groove', 'ridge', 'inset', 'outset']
      element.style.borderStyle = myborderStyles[Math.floor(Math.random() * myborderStyles.length)]
      element.style.borderColor = color[Math.floor(Math.random() * color.length)]
      this.myShadowRoot.appendChild(element)
    }
      // Make a span 
